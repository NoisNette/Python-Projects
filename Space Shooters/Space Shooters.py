...
def main():
	...
	while run:
		...
		if (keys[pygame.K_w] or keys[pygame.K_UP]) and player.y - player_vel > 0:  # Up
			player.y -= player_vel
		if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and player.y + player_vel + player.getHeight() + 10 < HEIGHT:  # Down
			player.y += player_vel
...
def mainMenu():
	...
	while run:
		win.blit(BG, (0, 0))
		title_label = title_font.render('Press any button to play...', 1, (255, 255, 255))
